...
module.exports = function (source) {
  ...
  const index = generateIndex({
    title: `${formattedName} | Pattern Library | ${store.state.config.project.name}`
  })
  ...
  nunjucksEnvironment.renderString(template, options, (error, render) => {
    ...
    this.emitFile(`${outputName}/index.html`, index)
    this.emitFile(`${outputName}/render.html`, render)
    this.emitFile(`${outputName}/source.html`, source)
