...
module.exports = function (src) {
  ...
  const previewHead = generatePreviewHead(store.config.project.base)
  ...
  const component = new Promise((resolve, reject) => {
    const options = {
      ...globalOptions,
      project: {
        name: (store.config.project && store.config.project.name) || 'Pangolin',
        theme: (store.config.project && store.config.project.theme)
      },
