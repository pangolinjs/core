...
module.exports = function (src) {
  ...
  try {
    pageConfig = JSON.parse(fs.readFileSync(pageConfigPath).toString())
  } catch (error) {}
  ...
  if (process.env.PANGOLIN_ENV === 'build:dev') {
    previewHead +=
      `<link rel="stylesheet" href="${projectBase}css/vendors.css">\n` +
      `<link rel="stylesheet" href="${projectBase}css/main.css">\n` +
      `<script defer src="${projectBase}js/vendors.js"></script>\n`
  }

  previewHead += `<script defer src="${projectBase}js/main.js"></script>`

  const previewTemplate =
    `{% extends "templates/${pageConfig.template || 'default.njk'}" %}` +
