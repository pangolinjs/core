...
module.exports = async function () {
  ...
  const serverPort = await getPort(store.config.devServer.port)
    ...
    .then(server => {
      ...
      server.on('build-finished', ({ stats, compiler }) => {
        ...
        if (isFirstCompile) {
          ...
          if (store.config.devServer.open) {
            opn(serverURL, { app: store.config.devServer.browser })
          }
        } else {
  ...
  chokidar
    .watch('src/**/*.{njk,md}', chokidarConfig)
